...
-- Seed Roles
INSERT INTO roles (role_name, description) VALUES ('customer', 'Regular bank customer with standard access') ON CONFLICT (role_name) DO NOTHING;
INSERT INTO roles (role_name, description) VALUES ('teller', 'Bank teller with counter operation access') ON CONFLICT (role_name) DO NOTHING;
INSERT INTO roles (role_name, description) VALUES ('admin', 'System administrator with full access') ON CONFLICT (role_name) DO NOTHING;
INSERT INTO roles (role_name, description) VALUES ('supervisor', 'Branch supervisor with approval authority') ON CONFLICT (role_name) DO NOTHING;

-- Assign customer role to all existing customers
INSERT INTO user_roles (cif, role_id, assigned_by) 
SELECT c.cif, r.id, 'SYSTEM' FROM customers c, roles r WHERE r.role_name = 'customer' ON CONFLICT DO NOTHING;

-- Assign admin role to CIF001 (Budi Santoso) for testing
INSERT INTO user_roles (cif, role_id, assigned_by) 
SELECT 'CIF001', r.id, 'SYSTEM' FROM roles r WHERE r.role_name = 'admin' ON CONFLICT DO NOTHING;

-- Assign supervisor role to CIF003 (Ahmad Wijaya) for testing  
INSERT INTO user_roles (cif, role_id, assigned_by) 
SELECT 'CIF003', r.id, 'SYSTEM' FROM roles r WHERE r.role_name = 'supervisor' ON CONFLICT DO NOTHING;
