...
func GetTransactionLimits(c *gin.Context) {
	...
	if roleName != "" {
		...
		limits, err = services.GetTransactionLimits(roleName)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"status":  "error",
				"message": err.Error(),
...
func UpdateTransactionLimit(c *gin.Context) {
	...
	if err := services.UpdateTransactionLimit(req.ID, req.DailyLimit, req.PerTransactionLimit, req.MonthlyLimit); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"status":  "error",
			"message": err.Error(),
		})
...
// GetUserRoles returns roles for a specific user or all roles (admin only)
